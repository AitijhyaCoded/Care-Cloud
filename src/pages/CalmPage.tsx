...
const CalmPage = () => {
  ...
  const allMeditations = [
    ...
    },
    {
      ...
      category: "Stress",
      forMood: [0, 1], // For low, down moods
    },
    {
    ...
    },
    {
      ...
      category: "Energy",
      forMood: [3, 4], // For good, great moods
    },
    {
  ...
  // Filter meditations based on mood
  const recommendedMeditations = allMeditations.filter(med => 
    med.forMood.includes(currentMood)
  );
  
  // Show all meditations if nothing is recommended for current mood
  const meditationsToShow = recommendedMeditations.length > 0 
  ...
  const getAffirmation = () => {
    ...
    // Different affirmations based on mood
    if (currentMood <= 1) {
      return affirmations[2]; // More supportive for low moods
    } else if (currentMood === 2) {
      return affirmations[0]; // Balanced for neutral mood
    } else {
      return affirmations[1]; // More energetic for good moods
    }
  };
    ...
    <MainLayout pageTitle="Find Calm">
      <div className="space-y-6 py-4">
        {currentMood <= 2 && (
          <Card className="p-4 bg-care-lightest border-care-light mb-4">
            <p className="text-sm text-care-text">
              We noticed your mood is <strong>{getMoodLabel()}</strong>. Here are some meditations that might help you feel better.
            </p>
          </Card>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button 
            className="h-auto py-4 px-3 bg-healing-lightest hover:bg-healing-light text-healing-dark"
          >
            <div className="flex flex-col items-center">
              <div className="p-3 rounded-full bg-white mb-2">
          ...
          <Card className="p-6 bg-gradient-to-b from-care-lightest to-comfort-lightest border-care-light shadow-sm animate-fade-in">
            <div className="flex flex-col items-center">
              ...
              <p className="text-sm text-center text-care-text mb-3">
                Follow the circle's rhythm.<br />Breathe in as it expands, out as it contracts.
              </p>
              <Button 
                onClick={toggleBreathing}
                variant="outline" 
        ...
        <div>
          <h2 className="text-lg font-medium text-care-text mb-3">
            {recommendedMeditations.length > 0 ? 'Recommended Meditations' : 'Guided Meditations'}
          </h2>
